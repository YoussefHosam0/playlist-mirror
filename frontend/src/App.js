...
function SongSearch() {
  ...
  return (
    <div className="card">
      ...
      <input
        placeholder="Song name (English or Arabic)"
        value={song}
      ...
      {result && (
        <div className="result">
          ...
          <div className="result-info">
            ...
            <div className="links">
              <a href={result.spotify_url} target="_blank" rel="noreferrer" className="btn spotify">
                Open on Spotify
              </a>
              <a href={result.anghami_url} target="_blank" rel="noreferrer" className="btn anghami">
                Search on Anghami
              </a>
            </div>
          </div>
...
function PlaylistMirror() {
  ...
  const normalize = (str) =>
    str.toLowerCase().replace(/[^a-z0-9\u0600-\u06FF\s]/g, "").trim();
  ...
  const sync = async () => {
    ...
    try {
      ...
      if (spotifyUrl.trim()) {
        const res = await axios.get(`${API}/playlist/spotify`, {
      ...
      if (anghamiUrl.trim()) {
        const res = await axios.get(`${API}/playlist/anghami`, {
          params: { url: anghamiUrl },
        });
        aTracks = res.data.tracks;
        setAnghamiTracks(aTracks);
      }

      const missingInSpotify = aTracks.filter(
        (at) =>
          !sTracks.some(
            (st) =>
              normalize(st.name).includes(normalize(at.name)) ||
              normalize(at.name).includes(normalize(st.name))
          )
      );

      const missingInAnghami = sTracks.filter(
        (st) =>
          !aTracks.some(
            (at) =>
              normalize(st.name).includes(normalize(at.name)) ||
  ...
  return (
    <div className="card">
      ...
      />
      <input
        placeholder="Anghami playlist URL"
        value={anghamiUrl}
        onChange={(e) => setAnghamiUrl(e.target.value)}
      />
        ...
        <div className="sync-results">
          ...
          {missing.missingInSpotify.length > 0 && (
            <div className="missing-section">
              ...
              <ul>
                {missing.missingInSpotify.map((t, i) => (
                  <li key={i} className="track-row">
                    <span dir="auto" className="track-title">{t.name}</span>
                    <span className="artist-tag" dir="auto">{t.artist}</span>
                    
                      href={`https://open.spotify.com/search/${encodeURIComponent(t.name + " " + t.artist)}`}
                      target="_blank"
                      rel="noreferrer"
          ...
          {missing.missingInAnghami.length > 0 && (
            <div className="missing-section">
              ...
              <ul>
                {missing.missingInAnghami.map((t, i) => (
                  <li key={i} className="track-row">
                    <span dir="auto" className="track-title">{t.name}</span>
                    <span className="artist-tag" dir="auto">{t.artist}</span>
                    
                      href={`https://play.anghami.com/search/${encodeURIComponent(t.name + " " + t.artist)}`}
                      target="_blank"
                      rel="noreferrer"
                      className="btn anghami small"
                    >
                      Find on Anghami
                    </a>
                  </li>
...
export default function App() {
  ...
  return (
    <div className="app">
      ...
      <main>
        {tab === "search" ? <SongSearch /> : <PlaylistMirror />}
      </main>
    </div>
  );
}